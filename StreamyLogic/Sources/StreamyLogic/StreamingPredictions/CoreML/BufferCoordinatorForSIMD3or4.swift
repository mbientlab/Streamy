...
///
class BufferedCoreMLCoordinatorForSIMD34: CoreMLClassifierCoordinator {
    ...
    ///
    init(model: MLModel, queue: DispatchQueue, vectorLabels: [String] = ["X", "Y", "Z", "W"]) throws {
        DispatchQueue.assertOnCoreMLQueue()

        self.model = model
        self.queue = queue
        self.vectorLabels = vectorLabels
        ...
        (self.inputFeatureNames, self.outputFeatureNames) = model.featureNames()
    ...
    /// Passed-in field names that map to a SIMD3/4 vector's indices (e.g., XYZW or roll/pitch/yaw)
    private let vectorLabels: [FeatureName]
    ...
    func add(datum: Any) {
        queue.async { [weak self] in
            if let datum = datum as? SIMD4<Float> {
                self?.add(simd: datum)

            } else if let datum = datum as? SIMD3<Float> {
                self?.add(simd: SIMD4.init(datum, 0))

            } else { self?.error.send(.invalidDataInputType) }
        }
    }
}

private extension BufferedCoreMLCoordinatorForSIMD34 {

    func add(simd vector: SIMD4<Float>) {
        DispatchQueue.assertOnCoreMLQueue()

        // For each value in the SIMD vector (e.g., XYZW)
        for i in vectorLabels.indices {
            let label = vectorLabels[i]
            let datum = vector[i]
...
extension BufferedCoreMLCoordinatorForSIMD34: ParentMLFeatureProvider {
